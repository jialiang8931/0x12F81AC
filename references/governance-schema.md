...
9. **交付邊界**：定案後是否需要 `build_deliverable_code.sh`、ZIP 保留與排除內容，以及只談業務邏輯和公開腳本用法的對外 `README.md`。
10. **預計變更**：精確 writable paths 與 forbidden paths。
11. **驗證與 rollback**：命令名稱、狀態與非破壞式回復。
12. **請使用者決定**：列出可直接刪改的規則與尚未核准項。
